...
| `enforcement` | No | Enforcement mode for inspected traffic: `enforce` or `audit`. The `interactive` object form (hold-and-ask) is only configurable via the YAML policy file — see the [policy schema reference](/reference/policy-schema#interactive-enforcement). |
